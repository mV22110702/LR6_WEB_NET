...
{
    public CompareDateTimeBaseAttribute(DateTime earliestDateTime)
    {
    ...
    {
        if (value == null) return new ValidationResult($"{validationContext.DisplayName} cannot be null");

        if (value is not DateTime) return new ValidationResult($"{OtherDateTimePropertyName} must be DateTime");
    ...
    {
        if (OtherDateTimePropertyName == null) return null;

        var otherProperty = validationContext.ObjectType.GetProperty(OtherDateTimePropertyName);
        if (otherProperty == null) return new ValidationResult($"Unknown property: {OtherDateTimePropertyName}");
    ...
    protected ValidationResult? Validate(object? value, ValidationContext validationContext)
    {
        ...
        if (valueValidationResult != null) return valueValidationResult;
        var otherPropertyValidationResult = ValidateOtherProperty(validationContext);
        if (otherPropertyValidationResult != null) return otherPropertyValidationResult;
        return null;
    }
